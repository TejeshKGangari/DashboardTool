...
curl_setopt_array($curl, array(
  CURLOPT_PORT => "8080",
  CURLOPT_URL => "http://tfs.ups.com:8080/tfs/UpsProd/_apis/wit/wiql?api-version=1.0",
  ...
  CURLOPT_POSTFIELDS => "{\"query\": \"Select [System.Id], [System.Title], [System.State] From WorkItems Where [ups.ApplicationCodes] = `CS Central Config System`",  //hardcoded the where clause
...
curl_setopt_array($curl, array(
  CURLOPT_PORT => "8080",
  CURLOPT_URL => "http://tfs.ups.com:8080/tfs/UpsProd/_apis/wit/workItems/?ids=".$pusharray1."&api_version=1.0",
  CURLOPT_RETURNTRANSFER => true,
  CURLOPT_ENCODING => "",
  CURLOPT_MAXREDIRS => 10,
  CURLOPT_TIMEOUT => 30,
	...
	if($array!='')
{
	...
	mysqli_query($con,"DELETE FROM tfsdefects where projectrelease = 'July 2016 Enterprise Release'")or die(mysqli_connect_errno()); //hardcoded the where clause

	$query = "INSERT INTO `tfsdefects`(`defectid`, `description`, `raiseddate`, `severity`, `priority`, `status`, `projectrelease`, `createdby`, `rootcause`, `closeddate`) VALUES";
	$where ="";
	 //echo $array['count'];
...
$where .="('".$defectid."','".$description."','".$raiseddate."','".$severity."','".$priority."','".$status."','".$projectrelease."','".$createdby."','".$rootcause."','".$closeddate."')";
		//print_r($array);
}
$query = $query.$where;
//echo $query;
